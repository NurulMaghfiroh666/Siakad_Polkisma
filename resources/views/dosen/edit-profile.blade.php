...
@section('title', 'Edit Profil - Dashboard Dosen')
...
<div class="card fade-in" style="max-width: 800px; margin: 0 auto; padding: 30px;">
    <form action="{{ route('dosen.profile.update') }}" method="POST">
        ...
        <div class="form-group mb-4">
            ...
            <input type="text" class="form-control" value="{{ $dosen->Nama ?? auth()->user()->name }}" disabled style="background-color: #f3f4f6;">
            <small class="text-muted">Nama tidak dapat diubah secara mandiri. Hubungi admin untuk perubahan.</small>
        </div>

        <div class="form-group mb-4">
            <label class="form-label">NIP</label>
            <input type="text" class="form-control" value="{{ $dosen->NIP ?? auth()->user()->login_id }}" disabled style="background-color: #f3f4f6;">
        </div>

        <div class="form-group mb-4">
            <label for="email" class="form-label">Email</label>
            <input type="email" name="email" id="email" class="form-control @error('email') is-invalid @enderror" value="{{ old('email', $dosen->Email ?? auth()->user()->email) }}" required>
            @error('email')
        ...
        <div class="form-group mb-4">
            ...
            <input type="text" name="telepon" id="telepon" class="form-control @error('telepon') is-invalid @enderror" value="{{ old('telepon', $dosen->Telepon ?? '') }}" required>
            @error('telepon')
                <div class="invalid-feedback" style="color: #ef4444; font-size: 0.875rem; margin-top: 5px;">{{ $message }}</div>
            @enderror
        </div>
        ...
        <div class="form-group mb-4">
            ...
            <textarea name="alamat" id="alamat" rows="3" class="form-control @error('alamat') is-invalid @enderror" required>{{ old('alamat', $dosen->Alamat ?? '') }}</textarea>
            @error('alamat')
                <div class="invalid-feedback" style="color: #ef4444; font-size: 0.875rem; margin-top: 5px;">{{ $message }}</div>
            @enderror
        </div>
