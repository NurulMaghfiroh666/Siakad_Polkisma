...
<div class="space-y-6">
    <div class="flex items-center">
        ...
        <div>
            ...
            <p class="text-gray-600 mt-1">Update jadwal mata kuliah</p>
        </div>
    </div>
    ...
    <div class="bg-white rounded-lg shadow-md p-6">
        <form action="{{ route('admin.jadwal.update', $jadwal->IdJadwal) }}" method="POST">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    ...
                    <select name="KodeMK" id="KodeMK" required class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-green-500 @error('KodeMK') border-red-500 @enderror">
                        ...
                        <option value="{{ $mk->KodeMK }}" {{ old('KodeMK', $jadwal->KodeMK) == $mk->KodeMK ? 'selected' : '' }}>
                ...
                <div>
                    ...
                    <select name="IdDosen" id="IdDosen" required class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-green-500 @error('IdDosen') border-red-500 @enderror">
                        ...
                        <option value="{{ $dosen->IdDosen }}" {{ old('IdDosen', $jadwal->IdDosen) == $dosen->IdDosen ? 'selected' : '' }}>
                            {{ $dosen->Nama }}
                        </option>
                        @endforeach
                    </select>
                ...
                <div>
                    ...
                    <input type="text" name="Ruang" id="Ruang" value="{{ old('Ruang', $jadwal->Ruang) }}" required class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-green-500 @error('Ruang') border-red-500 @enderror">
                    @error('Ruang')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div>
                    ...
                    <input type="time" name="JamMulai" id="JamMulai" value="{{ old('JamMulai', $jadwal->JamMulai) }}" required class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-green-500 @error('JamMulai') border-red-500 @enderror">
                    @error('JamMulai')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div>
                    ...
                    <input type="time" name="JamSelesai" id="JamSelesai" value="{{ old('JamSelesai', $jadwal->JamSelesai) }}" required class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-green-500 @error('JamSelesai') border-red-500 @enderror">
                    @error('JamSelesai')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
            ...
            <div class="mt-6 pt-6 border-t border-gray-200 flex justify-end space-x-4">
                ...
                <button type="submit" class="px-6 py-2 bg-gradient-to-r from-green-500 to-green-600 text-white rounded-lg hover:from-green-600 hover:to-green-700">Update</button>
            </div>
        </form>
    </div>
</div>
